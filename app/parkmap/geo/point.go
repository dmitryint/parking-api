...
func (p *Point) GreatCircleDistance(p2 *Point) float64 {
	dLat := p.degreesToRadians(p2.Lat() - p.Lat())
	dLon := p.degreesToRadians(p2.Lng() - p.Lng())

	lat1 := p.degreesToRadians(p.Lat())
	lat2 := p.degreesToRadians(p2.Lat())
...
func (p *Point) degreesToRadians(degrees float64) float64 {
